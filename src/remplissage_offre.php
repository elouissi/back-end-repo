...
if(isset($_POST['addskill'])) {
    ...
    $user_id = $_SESSION['id'];
    $query = "INSERT into `skills_of_freelancer` (`id`,`id_skills` ) 
    values ( $user_id, $id_skills  )";
...
<body class="overflow-x-hidden dark:bg-slate-900">
    ...
    <header class="flex bg-gray-50 justify-between p-2 md:py-3 md:px-8 lg:px-10 dark:bg-slate-800">
        ...
        <div class="flex">
            ...
            <span class="text-custom-green text-md md:text-xl lg:text-2xl font-bold self-center">People<span
                    class="text-gray-500 dark:text-white">Per</span>Task
            </span>
        </div>
        ...
        <div id="hamberger_btn" class="lg:hidden block self-center mr-4">
            <svg width="26" height="20" viewBox="0 0 26 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                <rect x="25.5" y="11" width="25" height="2.00004" rx="1.00002" transform="rotate(180 25.5 11)"
                    fill="#9a9a9a" />
                    ...
                    <article class="article">
                        ...
                        <div class="article-content">
                            ...
                            if(isset($_SESSION['role']) && ($_SESSION['role'] == "admin" || $_SESSION['role'] == "freelancer")):
                                ...
                                ?>
                                <button id="offer_btn"
                                    class="flex h-14 w-1/3  justify-center drop-shadow-md bg-custom-green- rounded-md border bg-opacity-50 border-gray-300 text-gray-50 items-center"
                                    style="
    margin-top: 62px;
"> créer l'offre</button>
                            ...
                            <div class="contact-form article-comment" style="margin: 10px">
                                ...
                                <form id="contact-form" method="POST">
                                    <div class="row">
                                        ...
                                        <p><strong>Le prix d'offer: </strong>
                                            <?php echo $row['Amount'] ?>
                                        </p>
                                        <p><strong>la date fin: </strong>
                                            <?php echo $row['Deadline'] ?>
                                        </p>
                                        <p><strong>le message d'offer: </strong>
                                            <?php echo $row['message'] ?>
                                        </p>
                                        <p><strong>le titre du project: </strong>
                                            <?php echo $row['titre'] ?>
                                        </p>
                                        <p><strong>le nom du freelancer: </strong>
                                            <?php echo $row['name'] ?>
                                        </p>
                                        <?php
                                        ...
                                        if($row['status'] == "pending"):
                                            ...
                                            <a href="modifier_status.php?Id_offer=<?php echo $Id_offer; ?>"
                                                class="btn flex items-center text-center  p-2 text-gray-900 rounded-lg   "
                                                style=" margin:auto;     background-color: #00FF0A;     width: 27%;">accepter</a>
                ...
                <div class="col-lg-6 m-15px-tb blog-aside float-right">
                    <!-- Author -->
                    <div class="widget widget-author">
                        ...
                        $query_skills = "SELECT
                        user.name,
                        skills_of_freelancer.id_skills,
                        skills.name_skills
                    FROM
                        skills_of_freelancer
                    JOIN
                        user ON skills_of_freelancer.id = user.id
                    JOIN
                        skills ON skills_of_freelancer.id_skills = skills.id_skills
                    GROUP BY
                        user.name, skills_of_freelancer.id_skills, skills.name_skills
                    LIMIT 0, 100";
                        $result_skills = mysqli_query($conn, $query_skills);
                        while($row = mysqli_fetch_assoc($result_skills)) {
                        ?>
                                <a href="#"><?php echo $row['name_skills'] ?></a>
                                <?php
                        }
                                ?>
                              
                                <input type="text" name="id" class="hidden" placeholder="<?php echo $_SESSION['id'] ?>"
                                    value="<?php echo $_SESSION['id'] ?>">
                                <form method="post">
                                    <select id="category" name="id_skills"
                                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-500 focus:border-primary-500 block w-1/2 p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                                        ...
                                        while($row = mysqli_fetch_assoc($result)):
                                            ...
                                            ?>
                                            <option value="<?= $row['id_skills'] ?>">
                                                <?= $row['name_skills']; ?>
                                            </option>
                                            <!-- <a href="ajouterdbskills.php?idskill="   
                                class=" btn-success ms-3 flex  text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">add skls</a> -->
                                        <?php endwhile; ?>

                                    </select>
                                    <button name="addskill" type="submit" id="btnmodal"
                ...
                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                    ...
                    <div class=" md:p-5 space-y-4">
                        <form action="">
                            ...
                            <div>
                                <input type="text" name="id" id="description"
                                    class="  hidden  bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500   w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 mb-4 dark:text-white"
                                    placeholder="<?php echo $_SESSION['id'] ?>" value="<?php echo $_SESSION['id'] ?>">
                            </div>
                            <div>
                                <input type="text" name="id_project" id="id_project"
                                    class=" hidden  bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500   w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 mb-4 dark:text-white"
                                    placeholder="<?php echo $_GET['id_project'] ?>"
                                    value="<?php echo $_GET['id_project'] ?>">
                            </div>
                            <div>
